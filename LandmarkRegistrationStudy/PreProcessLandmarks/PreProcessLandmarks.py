...
class PreProcessLandmarksWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    RepairInterfaceLayout.addWidget(qt.QLabel("Repaired"), 4, 3, 1, 1)
    ...
    self.RepairedLeftSelector.noneEnabled = True
    ...
    #self.AutocompleteLeftButton.enabled = True
    #RepairInterfaceLayout.addWidget(self.AutocompleteLeftButton, 5, 2, 1, 1)
    RepairInterfaceLayout.addWidget(self.RepairedLeftSelector, 5, 3, 1, 1)
    ...
    self.RepairedRightSelector.noneEnabled = True
    ...
    #self.AutocompleteRightButton.enabled = True
    #RepairInterfaceLayout.addWidget(self.AutocompleteRightButton, 6, 2, 1, 1)
    RepairInterfaceLayout.addWidget(self.RepairedRightSelector, 6, 3, 1, 1)
    ...
    self.RepairSideSelector.toolTip = "Use to select the node containing the spine side to be repaired"
    self.RepairSideSelector.setMRMLScene(slicer.mrmlScene)
    RepairInterfaceLayout.addWidget(qt.QLabel("Side to repair:"), 8, 0, 1, 1)
    RepairInterfaceLayout.addWidget(self.RepairSideSelector, 8, 1, 1, 3)
    
    self.AutoCompleteButton = qt.QPushButton("Auto-Complete")
    self.AutoCompleteButton.toolTip = "Guess and impute missing landmarks"
  ...
  def OnCategorizeLeftRight(self):
    ...
    (LeftSide, RightSide) = self.logic.CategorizeLeftRight()
  ...
  def AutoComplete(self):
    ...
    if self.NodeToRepair.GetName().__contains__("Left"):
      #RepairedNode.Copy(self.logic.LeftSide.PredictAndImputeOmissions())
      #RepairedNode = slicer.mrmlScene.CopyNode(self.logic.LeftSide.PredictAndImputeOmissions())
      #slicer.mrmlScene.AddNode(RepairedNode)
      NewNode = (self.logic.LeftSide.PredictAndImputeOmissions())
      slicer.mrmlScene.AddNode(NewNode)
      self.RepairedLeftSelector.setCurrentNode(NewNode)
      self.RepairedLeftSelector.currentNode().GetDisplayNode().SetSelectedColor(0,0,255)
    elif self.NodeToRepair.GetName().__contains__("Right"):
      #RepairedNode.Copy(self.logic.RightSide.PredictAndImputeOmissions())
      NewNode = (self.logic.RightSide.PredictAndImputeOmissions())
      slicer.mrmlScene.AddNode(NewNode)
      self.RepairedRightSelector.setCurrentNode(NewNode)
      self.RepairedRightSelector.currentNode().GetDisplayNode().SetSelectedColor(0,0,255)
  ...
  def AutocompleteLeft(self):
    ...
    self.RepairedLeftSelector.setCurrentNode(RepairedLeftNode)
  ...
  def AutocompleteRight(self):
    ...
    self.RepairedRightSelector.setCurrentNode(RepairedRightNode)
  ...
  def OnRepairSideSelected(self):
    self.NodeToRepair = self.RepairSideSelector.currentNode()
    if self.NodeToRepair != None:
      self.NodeToRepair.GetDisplayNode().SetSelectedColor(255,0,0)
...
class PreProcessLandmarksLogic(ScriptedLoadableModuleLogic):
  ...
  class SpineSide:
    def __init__(self, parent, Node):
      self.MarkupsNode = slicer.vtkMRMLMarkupsFiducialNode()
      self.MarkupsNode.Copy(Node)
      self.RepairedMarkupsNode = slicer.vtkMRMLMarkupsFiducialNode()
      self.RepairedMarkupsNode.SetName(parent.MarkupsNode.GetName() + "_Rep")
    ...
    def OrderPointsSuperiorToInferior(self, Node):
      ...
      for i, Markup in enumerate(LabelsCoords):
        Node.AddFiducialFromArray(Markup[1])
        Node.SetNthFiducialLabel(i, Markup[0])
    ...
    def EstimateOmissions(self, IntervalIndex, IntervalData):   # Finds and adds the optimum number of points to a given interval to minimize the entire curve's frequency fit RMS
      ...
      # Try 2 or more imputations in the interval, see if the freq. fit improves substantially
      ...
      RepairedMarkupsNode = slicer.vtkMRMLMarkupsFiducialNode()
      RepairedMarkupsNode.Copy(TentativeNode)

      PriorRepairedNode = slicer.util.getNode(self.MarkupsNode.GetName() + "_Rep")
      if PriorRepairedNode != None:
        slicer.mrmlScene.RemoveNode(PriorRepairedNode)
      RepairedMarkupsNode.SetName(self.MarkupsNode.GetName() + "_Rep")
      self.OrderPointsSuperiorToInferior(RepairedMarkupsNode)

      return RepairedMarkupsNode
      
    def PredictAndImputeOmissions(self):
      """
      if slicer.util.getNode(self.MarkupsNode.GetName() + "_Rep") != None:
        slicer.mrmlScene.RemoveNode(slicer.util.getNode(self.MarkupsNode.GetName() + "_Rep"))
        self.RepairedMarkupsNode = slicer.vtkMRMLMarkupsFiducialNode()
        self.RepairedMarkupsNode.SetName(self.MarkupsNode.GetName() + "_Rep")
      else:
        self.RepairedMarkupsNode = slicer.mrmlScene.CopyNode(self.MarkupsNode)
        self.RepairedMarkupsNode.SetName(self.MarkupsNode.GetName() + "_Rep")
      """
      ...
      while ImputingPoints:
        ...
        for i, (IntervalDatum, FitError) in enumerate(zip(CurvewiseInterpointData, FrequencyFitErrors)):
          if not SkipIntervals[i] and self.IdentifyOmissionsFromIntervalLengthFit(i, FrequencyFitErrors):
            print "Interval " + str(i) + " apparently missing points"
            ImputingPoints = True
            self.RepairedMarkupsNode = self.EstimateOmissions(i, CurvewiseInterpointData)
            NumImputations = self.RepairedMarkupsNode.GetNumberOfFiducials() - self.MarkupsNode.GetNumberOfFiducials()
            SkipIntervals = np.insert(SkipIntervals, i, np.ones(NumImputations+1))
            
            self.MarkupsNode.Copy(self.RepairedMarkupsNode)
    ...
    def ImputeMarkups(self, IntervalData, IntervalIndex, NumNewPoints, SrPolynomial, SaPolynomial, FrequencyPolynomial): # Returns a node with NumNewPoints points added on the curve in the specified interval, based on polynomial interpolation
      ...
      self.OrderPointsSuperiorToInferior(self.MarkupsNode)
      self.LabelsCoords = [(self.MarkupsNode.GetNthFiducialLabel(i), self.MarkupsNode.GetMarkupPointVector(i,0)) for i in range(self.MarkupsNode.GetNumberOfFiducials())]
      #return Node
    """
